...
const COINBASE_AMOUNT: number = 1;
...
const getTransactionId = (transaction: Transaction): string => {
  ...
  const transactionHash = CryptoJS.SHA512(txInContent + txOutContent).toString();
  const transactionHashDoubled = CryptoJS.SHA256(transactionHash).toString();

  return transactionHashDoubled;
};
...
const validateCoinbaseTx = (transaction: Transaction, blockIndex: number): boolean => {
  ...
  if (transaction.txOuts.length !== 1) {
    // If Genesis its ok othervise false
    if (blockIndex === 0) {
      return true;
    }
...
const validateTxIn = (txIn: TxIn, transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]): boolean => {
  ...
  const key = ec.keyFromPublic(bytes.toString('hex'), 'hex');
...
const getFigBaseTransaction = (address: string, blockIndex: number): Transaction => {
...
const getPublicKey = (aPrivateKey: string): string => {
  const key = ec.keyFromPrivate(aPrivateKey, 'hex');
...
const isValidAddress = (address: string): boolean => {
  ...
  if (address.length > 50 && address.length < 40) {
    console.log(address, 'node public key');
...
export {
  ...
  UnspentTxOut, TxIn, TxOut, getFigBaseTransaction, getPublicKey, hasDuplicates,
  Transaction
};
